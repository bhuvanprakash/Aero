...
class AeroWriter:
    ...
    * :meth:`write_model` – high-level, streams tensor data with
    ...
    def write(
        ...
        *,
        atomic: bool = True,
        add_control_integrity: bool = False,
    ) -> None:
        dest = out_path
        tmp_path = out_path + ".tmp" if atomic else out_path

        if add_control_integrity:
            # Prepend optional IHSH chunk (AIP-0001). Payload filled after we write header+TOC+string table.
            ihsh_placeholder = _make_ihsh_placeholder()
            chunks = [ihsh_placeholder] + list(chunks)
        ...
        for chunk in chunks:
            raw = chunk.data
            blake3_hashes.append(_blake3.digest_required(raw))
        ...
        # If first chunk is IHSH, we write header+TOC+string table, then compute control hash and fill IHSH.
        first_is_ihsh = len(chunks) > 0 and chunks[0].fourcc == FOURCC_IHSH
        try:
            with open(tmp_path, "wb") as f:
                ...
                if first_is_ihsh:
                    # Read back control region and compute IHSH digest (AIP-0001)
                    f.flush()
                    with open(tmp_path, "rb") as r:
                        control_region = r.read(payloads_start)
                    if len(control_region) != payloads_start:
                        raise ValueError("short read of control region")
                    control_digest = _blake3.digest_required(control_region)
                    ihsh_payload = msgpack.packb({
                        "algo": "blake3-256",
                        "covered_ranges": [[0, payloads_start]],
                        "digest": control_digest,
                    }, use_bin_type=True)
                    ihsh_digest = _blake3.digest_required(ihsh_payload)
                    stored_payloads[0] = ihsh_payload
                    blake3_hashes[0] = ihsh_digest
                    # Patch TOC entry 0 (IHSH) with correct BLAKE3
                    blake3_off_in_entry = TOC_ENTRY_SIZE - 32
                    f.seek(toc_start + TOC_HEADER_SIZE + blake3_off_in_entry)
                    f.write(ihsh_digest)
    ...
    def write_model(
        ...
        *,
        ...
        add_control_integrity: bool = False,
        add_page_hashes: bool = False,
        page_hash_page_size: int = PHSH_PAGE_SIZE_DEFAULT,
    ) -> None:
        ...
        *tensor_specs* is an iterable of :class:`TensorSpec`.  Each
        ...
        # ── Build manifest ───────────────────────────────────────────────
        toc_base = 1 if add_control_integrity else 0
        chunks_desc = []
        if add_control_integrity:
            chunks_desc.append({"role": "control_integrity", "fourcc": "IHSH", "toc_index": 0})
        chunks_desc.extend([
            {"role": "json", "fourcc": "MJSN", "toc_index": toc_base + 0},
            {"role": "manifest", "fourcc": "MMSG", "toc_index": toc_base + 1},
            {"role": "tensors", "fourcc": "TIDX", "toc_index": toc_base + 2},
        ])
        shard_infos = []
        next_toc = toc_base + 3
        for idx, (sid, shard_specs) in enumerate(shard_plan):
            chunks_desc.append({
                ...
                "toc_index": next_toc, "shard_id": sid,
            })
            next_toc += 1
            if add_page_hashes:
                chunks_desc.append({
                    "role": "page_hashes", "fourcc": "PHSH",
                    "toc_index": next_toc, "shard_id": sid,
                })
                next_toc += 1
            wtsh_toc_idx = next_toc - 2 if add_page_hashes else next_toc - 1
            shard_infos.append({
                "shard_id": sid, "toc_index": wtsh_toc_idx,
        ...
        _blake3.required_for_writing()
        b3 = _blake3.get_module()
        compressor = zstd.ZstdCompressor(level=6)
        processed: list[tuple] = []          # (fourcc, name, stored, ulen, flags, hash)
        if add_control_integrity:
            ihsh_pl = _make_ihsh_placeholder()
            raw = ihsh_pl.data
            processed.append((
                FOURCC_IHSH, ihsh_pl.name, raw, len(raw),
                ihsh_pl.flags, b"\x00" * 32,
            ))
        for c in meta_chunks:
            raw = c.data
            h = b3.blake3(raw).digest()
            flags = c.flags
            if c.compress_zstd:
                stored = compressor.compress(raw)
        ...
        # ── PHSH size estimates (for layout when add_page_hashes) ───────
        def _phsh_payload_size(shard_size: int, shard_name: str) -> int:
            n_pages = (shard_size + page_hash_page_size - 1) // page_hash_page_size
            # msgpack: map header + "shard_name" + name + "page_size" + u64 + "digests" + array + n*32
            return 80 + len(shard_name) + 32 * n_pages

        phsh_sizes: list[int] = []
        if add_page_hashes:
            for _, wname, sz, *_ in wtsh_list:
                phsh_sizes.append(_phsh_payload_size(sz, wname))

        # ── String table ─────────────────────────────────────────────────
        if add_page_hashes:
            all_names = [p[1] for p in processed]
            for _, wname, *_ in wtsh_list:
                all_names.append(wname)
                all_names.append(wname + ".phsh")
        else:
        ...
        # ── Layout ───────────────────────────────────────────────────────
        n_total = len(processed) + len(wtsh_list) + (len(phsh_sizes) if add_page_hashes else 0)
        ...
        for _, _, stored, *_ in processed:
            ...
            offsets.append(cur)
            cur += len(stored)
        for wi, (_, _, sz, *_ ) in enumerate(wtsh_list):
            cur = align16(cur)
            offsets.append(cur)
            cur += sz
            if add_page_hashes:
        ...
        try:
            with open(tmp_path, "wb") as f:
                ...
                # TOC entries: write all hashes as zero so IHSH digest is over deterministic region
                ...
                for i in range(n_total):
                    ...
                    if i < len(processed):
                        ...
                        j = i - len(processed)
                        if add_page_hashes:
                            k, is_phsh = divmod(j, 2)
                            if is_phsh:
                                fourcc = FOURCC_PHSH
                                flags = int(ChunkFlags.CHUNK_IS_OPTIONAL)
                                slen = ulen = phsh_sizes[k]
                            else:
                                fourcc, _, sz, flags, _ = wtsh_list[k]
                                slen, ulen = sz, sz
                        else:
                            fourcc, _, sz, flags, _ = wtsh_list[j]
                            slen, ulen = sz, sz
                    f.write(struct.pack(
                        TOC_ENTRY_FMT, fourcc, flags, offsets[i],
                        slen, ulen, name_ents[i][0], name_ents[i][1], 0, b"\x00" * 32,
                    ))
                ...
                # IHSH: close so control region is visible to a fresh read; then reopen to patch + write payloads
                if add_control_integrity:
                    f.flush()
                    os.fsync(f.fileno())

            with open(tmp_path, "r+b") as f:
                # Write each payload at its offset; patch TOC hashes except IHSH (patched after we compute digest)
                for i, (_, _, stored, ulen, flags, h) in enumerate(processed):
                    f.seek(offsets[i])
                    f.write(stored)
                    if i > 0:
                        f.seek(toc_positions[i] + _BLAKE3_OFF)
                        f.write(h)

                # WTSH + optional PHSH payloads – streamed by page when add_page_hashes
                tensor_hashes: dict[str, str] = {}  # name -> hex hash
                for wi, (_, wname, _, _, shard_specs) in enumerate(wtsh_list):
                    ci = len(processed) + (2 * wi if add_page_hashes else wi)
                    f.seek(offsets[ci])
                    shard_hasher = b3.blake3()
                    ...
                    if add_page_hashes:
                        ...
                        for spec in shard_specs:
                            ...
                            page_buf.extend(data)
                            while len(page_buf) >= page_hash_page_size:
                                page = bytes(page_buf[:page_hash_page_size])
                                del page_buf[:page_hash_page_size]
                                page_digests.append(b3.blake3(page).digest())
                                f.write(page)
                                shard_hasher.update(page)
                            if tensor_hash:
                                tensor_hashes[spec.name] = b3.blake3(data).hexdigest()
                            del data
                        if page_buf:
                            page = bytes(page_buf)
                            page_digests.append(b3.blake3(page).digest())
                            f.write(page)
                            shard_hasher.update(page)
                        # Write PHSH chunk (AIP-0002)
                        phsh_payload = msgpack.packb({
                            "shard_name": wname,
                            "page_size": page_hash_page_size,
                            "digests": page_digests,
                        }, use_bin_type=True)
                        phsh_digest = b3.blake3(phsh_payload).digest()
                        _pad_to_offset(f, offsets[ci + 1])
                        if len(phsh_payload) > phsh_sizes[wi]:
                            raise ValueError(
                                f"PHSH payload larger than reserved layout "
                                f"({len(phsh_payload)} > {phsh_sizes[wi]})"
                            )
                        f.write(phsh_payload)
                        # Patch TOC so chunk length matches actual payload (reader uses this)
                        f.seek(toc_positions[ci + 1] + 16)
                        f.write(struct.pack("<Q", len(phsh_payload)))
                        f.seek(toc_positions[ci + 1] + 24)
                        f.write(struct.pack("<Q", len(phsh_payload)))
                        end = f.tell()
                        f.seek(toc_positions[ci + 1] + _BLAKE3_OFF)
                        f.write(phsh_digest)
                        f.seek(end)
                    else:
                        for spec in shard_specs:
                            ...
                            if tensor_hash:
                                tensor_hasher = b3.blake3(data)
                                tensor_hashes[spec.name] = tensor_hasher.hexdigest()
                            del data
                        end = f.tell()
                    # Patch WTSH BLAKE3 hash in TOC entry
                ...
                if tensor_hash and tensor_hashes:
                    ...
                    tidx_hash = b3.blake3(tidx_with_hashes).digest()
                    # Find TIDX TOC position and update
                    tidx_toc_idx = 3 if add_control_integrity else 2  # TIDX position in processed
                ...
                # IHSH (AIP-0001): compute digest over control region after all other TOC patches
                if add_control_integrity:
                    f.seek(0)
                    control_region = bytearray(f.read(pay_start))
                    if len(control_region) != pay_start:
                        raise ValueError("short read of control region")
                    (_, _, _, _, toc_off_file, _, _, _, _, _, _) = struct.unpack(
                        HEADER_FMT, bytes(control_region[:HEADER_SIZE])
                    )
                    toc_hdr = bytes(control_region[toc_off_file : toc_off_file + TOC_HEADER_SIZE])
                    entry_count = struct.unpack(TOC_HEADER_FMT, toc_hdr)[0]
                    for i in range(entry_count):
                        ent = toc_off_file + TOC_HEADER_SIZE + i * TOC_ENTRY_SIZE
                        control_region[ent + TOC_ENTRY_SIZE - 32 : ent + TOC_ENTRY_SIZE] = b"\x00" * 32
                    _zeroed = bytes(control_region)
                    control_digest = _blake3.digest_required(_zeroed)
                    _debug_path = os.environ.get("AERO_DEBUG_IHSH_WRITER")
                    if _debug_path:
                        with open(_debug_path, "wb") as _dbg:
                            _dbg.write(_zeroed)
                    ihsh_payload = msgpack.packb({
                        "algo": "blake3-256",
                        "covered_ranges": [[0, pay_start]],
                        "digest": control_digest,
                    }, use_bin_type=True)
                    if len(ihsh_payload) > IHSH_PAYLOAD_SIZE:
                        raise ValueError("IHSH payload exceeds fixed size")
                    ihsh_payload = ihsh_payload.ljust(IHSH_PAYLOAD_SIZE, b"\x00")
                    ihsh_digest = _blake3.digest_required(ihsh_payload)
                    processed[0] = (
                        FOURCC_IHSH, "control.integrity", ihsh_payload,
                        len(ihsh_payload), int(ChunkFlags.CHUNK_IS_OPTIONAL), ihsh_digest,
                    )
                    f.seek(offsets[0])
                    f.write(ihsh_payload)
                    f.seek(toc_positions[0] + _BLAKE3_OFF)
                    f.write(ihsh_digest)
...
# Fixed IHSH payload size so TOC length need not be patched (digest only zeroes hash)
IHSH_PAYLOAD_SIZE = 256

def _make_ihsh_placeholder() -> Chunk:
    """Create IHSH chunk placeholder (AIP-0001). Payload filled at write time."""
    payload = msgpack.packb({
        "algo": "blake3-256",
        "covered_ranges": [[0, 0]],
        "digest": b"\x00" * 32,
    }, use_bin_type=True)
    payload = payload.ljust(IHSH_PAYLOAD_SIZE, b"\x00")
    return Chunk(
        fourcc=FOURCC_IHSH,
