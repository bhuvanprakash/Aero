    ...
    for (uint32_t i = 0; i < rmap.size; ++i) {
        ...
        for (uint32_t j = 0; j < arr.size; ++j) {
            ...
            for (uint32_t k = 0; k < tmap.size; ++k) {
                ...
                } else if (tkey == "quant_params" && val.type == msgpack::type::MAP) {
                    const auto& qmap = val.via.map;
                    for (uint32_t q = 0; q < qmap.size; ++q) {
                        std::string qk = mp_to_str(qmap.ptr[q].key);
                        if (qk == "ggml_type") {
                            te.ggml_type = static_cast<uint32_t>(mp_to_u64(qmap.ptr[q].val));
                            break;
                        }
                    }
                }
